...
export function buildAiRecipeSystemInstruction() {
  return [
    ...
    "- YouTube metadata (youtubeMetadata): populate duration from the video when available (VERIFIED). Generate 5–10 consolidated recipe chapters in youtubeMetadata.chapters with MM:SS times grounded in the video (VERIFIED for times, HIGH_CONFIDENCE_INFERENCE for summarized labels). When the video description lists timestamped chapters (e.g. 0:42 Step name), use those times as VERIFIED chapters but prefer clear procedural labels (mix, knead, proof, shape, cook, serve) over promotional titles. Cover major cooking stages actually shown — not one chapter per instruction line. Do not invent duration or timestamps.",
